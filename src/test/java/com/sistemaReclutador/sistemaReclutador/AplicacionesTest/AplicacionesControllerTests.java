...
// PATCH /aplicaciones/estado/{idPost}
	...
	void cambiarEstado_DebeRetornarNoContent() throws Exception {
		...
		mockMvc.perform(patch("/aplicaciones/estado/1").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(body))).andExpect(status().isNoContent());
		verify(aplicacionService, times(1)).cambiarEstado(1, true);
	}
	...
	void cambiarEstado_DebePermitirEstadoFalse() throws Exception {
		...
		mockMvc.perform(patch("/aplicaciones/estado/1").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(body))).andExpect(status().isNoContent());
		verify(aplicacionService, times(1)).cambiarEstado(1, false);
	}
...
// DELETE /aplicaciones/{id}
	...
	void deleteAplicacion_DebeRetornarNoContent() throws Exception {
		doNothing().when(aplicacionService).deleteById(1);
		mockMvc.perform(delete("/aplicaciones/1")).andExpect(status().isNoContent());
		verify(aplicacionService, times(1)).deleteById(1);
	}
...
// GET /aplicaciones/perfil/{idPerfil}
	...
	void obtenerAsignaciones_DebeRetornarOk() throws Exception {
		AplicacionesMiasResponse respuesta = new AplicacionesMiasResponse();
		when(aplicacionService.obtenerAplicacionesPerfil(eq(24), anyString())).thenReturn(List.of(respuesta));
		mockMvc.perform(get("/aplicaciones/perfil/24")).andExpect(status().isOk())
				.andExpect(jsonPath("$.length()").value(1));
		verify(aplicacionService, times(1)).obtenerAplicacionesPerfil(eq(24), anyString());
	}
...
void obtenerAsignaciones_DebeRetornarListaVacia() throws Exception {
	...
	void obtenerAsignaciones_DebeAceptarOtroFormato() throws Exception {
		AplicacionesMiasResponse respuesta = new AplicacionesMiasResponse();
		when(aplicacionService.obtenerAplicacionesPerfil(eq(24), eq("otroFormato"))).thenReturn(List.of(respuesta));
		mockMvc.perform(get("/aplicaciones/perfil/24").param("format", "otroFormato")).andExpect(status().isOk())
				.andExpect(jsonPath("$.length()").value(1));
		verify(aplicacionService, times(1)).obtenerAplicacionesPerfil(eq(24), eq("otroFormato"));
	}
